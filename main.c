...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
        ...
        case WM_PAINT :
            ...
            int **routes = create_xy_array(26,2);
            for (int i = 0; i < 26; ++i) {
                for (int j = 0; j < 2; ++j) {
                    routes[i][j] = ruts[i][j];
                }
            ...
            for (int k = 0; k < 26; k++){
                ...
                if (i == j) {
                    ...
                    Broken_arrow(hdc, A_cords[i][0], A_cords[i][1], A_cords[j][0], A_cords[j][1]);
                } else if (fabs(A_cords[i][0] - A_cords[j][0]) == edge &&
                           fabs(A_cords[i][1] - A_cords[j][1]) > height) {
                    Broken_arrow(hdc, A_cords[i][0], A_cords[i][1], A_cords[j][0], A_cords[j][1]);
                } else if (fabs(A_cords[i][0] - A_cords[j][0]) == half_edge * 3 &&
                           fabs(A_cords[i][1] - A_cords[j][1]) > height) {
                    Broken_arrow(hdc, A_cords[i][0], A_cords[i][1], A_cords[j][0], A_cords[j][1]);
                } else {
                    //normal
